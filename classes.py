...
class Level(Scene):
    ...
    def __init__(self, level_no, rows=ROWS, cols=COLS):
        ...
        self.player_position = self.load()
...
class Player(PositionedSprite):
    ...
    def move(self):
        ...
        if pygame.sprite.collide_rect(self, Exit.current):
            return True
...
class Game:
    ...
    def draw(self):
        ...
        if Player.player.alive():
            ...
            self.draw_text(
                f"health: {Player.player.health}", "black", TILE_SIZE, TILE_SIZE * 0.5
            )
